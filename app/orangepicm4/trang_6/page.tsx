...
export default function Trang6() {
  ...
  useEffect(() => {
    ...
    const colors = ["#3CC157", "#2AA7FF", "#1B1B1B", "#FCBC0F", "#F85F36"];
    const numBalls = 30; // Reduced number for better performance
    const balls = [];
    ...
    for (let i = 0; i < numBalls; i++) {
      ...
      ball.style.left = `${Math.floor(Math.random() * 100)}vw`;
      ball.style.top = `${Math.floor(Math.random() * 100)}vh`;
      ball.style.transform = `scale(${Math.random() * 0.5 + 0.5})`; // Adjusted scale
      ball.style.width = `${Math.random() * 0.5 + 0.5}em`;
      ball.style.height = ball.style.width;
      ball.style.opacity = "0.6"; // Added transparency
      ball.style.borderRadius = "50%"; // Ensure perfect circles
    ...
    balls.forEach((el, i) => {
      ...
      el.animate(
        ...
        {
          duration: (Math.random() + 1) * 3000, // Slower animation
          direction: "alternate",
          fill: "both",
          iterations: Infinity,
          easing: "ease-in-out",
  ...
  return (
    <AnimatePresence>
      <motion.div
        className="min-h-screen flex flex-col select-none relative bg-gradient-to-b from-gray-50 to-gray-100"
      ...
      >
        ...
        {/* Main Content */}
        <div className="flex flex-col items-center justify-center mt-72 px-4">
          <motion.div
            className="text-[200px] text-red-500 bonheur-royale-regular text-center"
        ...
        {/* Footer */}
        <motion.div
          className="fixed bottom-0 left-0 right-0 text-center mb-4 bg-white/50 backdrop-blur-sm py-4"
          initial={{ y: 50, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          transition={{ delay: 1 }}
        >
